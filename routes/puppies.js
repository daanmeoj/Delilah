...
router.get("/", (req, res, next) => {
  ...
  Puppy.findAll({
    where: req.query,
    include: [{ model: Food, as: "favFoods" }],
  })
...
router.put("/:id/food", (req, res, next) => {
  req.puppy.addFavFoods(req.body.foodId).then(res.send.bind(res)).catch(next);
});

// router.put("/:id/food", (req, res, next) => {
//   Puppy.findById(req.params.id)
//     .then((puppy) => {
//       return puppy.addFavFoods(req.body.foodId);
//     })
//     .then(res.send.bind(res))
//     .catch(next);
// });
...
router.post("/:id/food", (req, res, next) => {
  req.puppy.createFavFood(req.body).then(res.send.bind(res)).catch(next);
});
